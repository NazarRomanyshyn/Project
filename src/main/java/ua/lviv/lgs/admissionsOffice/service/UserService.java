...
public class UserService implements UserDetailsService {
	...
	public void sendActivationCode(User user) {
		...
		if (!StringUtils.isEmpty(user.getEmail())) {
            String message = String.format(
            		"Вітаємо, %s %s! \n\n" +
            				"Ласкаво просимо \"приймальна комісія\".\n" +
            				"Для продовження регістрації і активації свого акаунта перейдіть, будь ласка, за ардесою:\n" +
            ...
            mailSender.send(user.getEmail(), "Код активації акаунта", message);
        }
	}
    ...
    public Map<String, String> getUserErrors(Map<String, String> form) {
    	...
    	if (StringUtils.isEmpty(form.get("firstName"))) {
    		errors.put("firstNameError", "Ім'я користувача не може бути пустим!");			
    	}
    	
    	if (StringUtils.isEmpty(form.get("lastName"))) {
    		errors.put("lastNameError", "Прізвище користувача не може бути пустим!");
    	}
    	return errors;
    }
    ...
    public Map<String, String> getProfileErrors(User user, String firstName, String lastName, String email,
    	...
    	if (StringUtils.isEmpty(email)) {
    		errors.put("emailError", "Email користувачаля не може бути пустим!");
    	}
    	
    	if (password.length() < 6) {
    		errors.put("passwordError", "Пароль користувачаповинен має мати не менше 6 символів!");
    	}
    	
    	if (confirmPassword.length() < 6) {
    		errors.put("confirmPasswordError", "Пароль користувачаповинен має мати не менше 6 символів!");
    	}
    	
    	if (password != "" && confirmPassword != "" && !password.equals(confirmPassword)) {
    		errors.put("confirmPasswordError", "Ведені паролі не співпадають!");
    	}
