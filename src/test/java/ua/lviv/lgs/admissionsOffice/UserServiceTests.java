...
public class UserServiceTests {
	...
	public void saveUserTest() {
		User user = new User("Василь", "Василенко", "vasilenko@mail.com", "123456", true, new HashSet<AccessLevel>(Collections.singleton(AccessLevel.USER)));
		Map<String, String> form = new HashMap<String, String>() {
			private static final long serialVersionUID = 346045069597473933L;

		{
			put("firstName", "Іван");
			put("lastName", "Петренко");
			put("ADMIN", "ADMIN");
		}};		
		
		userService.saveUser(user, form);
		
		Assert.assertTrue(user.getFirstName().equals("Іван"));
		Assert.assertTrue(user.getLastName().equals("Петренко"));
		Assert.assertFalse(user.isActive());
